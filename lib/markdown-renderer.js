...
function codeBlockquote(quote) {
  ...
  if(cite) {
    if(link) {
      return `<blockquote class="with-cite">${core}<cite><a href="http://${link}" target="blank">${cite}</a></cite></blockquote>`;
    } else {
      return `<blockquote class="with-cite">${core}<cite>${cite}</cite></blockquote>`;
    }
  } else {
    return `<blockquote>${core}</blockquote>`;
  }
}
